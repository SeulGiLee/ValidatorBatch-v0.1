...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    def do_join(self):
        ...
        if self.dockwidget.path1.text() =='' or self.dockwidget.path2.text()=='' or self.dockwidget.path3.text() =='':
            ...
            #QgsMessageLog.logMessage(u"qString --> %s" % query)
            #QMessageBox.warning(self.iface.mainWindow(), u'dd', query)
            # subprocess.call(['java', '-jar','C:/val/val.jar' ,'c:\\textMining.txt'])
            #os.system(query)
            #os.system("C:/val/start.bat")
            #os.system('cmd.exe')
            #subprocess.run('C:/val/start_.bat')
            #jd = subprocess.check_output(["java", "-version"],stderr=subprocess.STDOUT)
            #v = '결과' + jd
            os.system('C:/val/start.bat')
            #os.system(String)
            #version = subprocess.check_output(['java', '-version'], stderr=subprocess.STDOUT)
            #if version >= 1.8: QMessageBox.warning(self.iface.mainWindow(), u'알림', 'ddd')
            #else :  QMessageBox.warning(self.iface.mainWindow(), u'알림', 'd')
            # subprocess.Popen(String, shell=True)
            #QMessageBox.warning(self.iface.mainWindow(), u'알림', v)
            # os.system(String)
